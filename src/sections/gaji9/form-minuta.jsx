...
export default function MinutaForm({ onClose, action, minuta = null }) {
  ...
  const onSubmit = async () => {
    const formData = {
      ...
      ...(action === 'Atualizar'
        ? { ativo: values?.ativo }
        : { tipos_garantias: values?.garantias?.map(({ garantia }) => garantia?.id) }),
    };
    if (action === 'Adicionar') {
...
export function GarantiasForm({ onClose }) {
  ...
  const onSubmit = async () => {
    dispatch(
      updateItem(
        'garantiasMinuta',
        JSON.stringify({ tipos_garantias: values?.garantias?.map(({ garantia }) => garantia?.id) }),
...
export function ComponentesForm({ onClose }) {
  ...
  const onSubmit = async () => {
    const ids = values?.componentes?.map(({ componente }) => ({ componente_id: componente?.id }));
      ...
      clausulas:
        action === 'compor'
          ...
          : minuta?.clausulas
              ...
              ?.map(({ numero_ordem: num, clausula_id: cid, titulo }) => ({
                numero: num,
                clausula: { id: cid, label: titulo || 'Cláusula solta' },
              })),
    }),
    [action, minuta]
  );
