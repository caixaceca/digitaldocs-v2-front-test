...
export function EntradasTrabalhados({ indicadores }) {
  ...
  const colaboradoresAcesso = useMemo(
    () => ColaboradoresAcesso(colaboradores, cc, isAdmin || isAuditoria, meusAmbientes)?.map(({ id }) => id),
    [cc, colaboradores, isAdmin, isAuditoria, meusAmbientes]
  );
  const dadosByColaborador = useMemo(
    () =>
      indicadoresGroupBy(
        indicadores?.filter(({ perfil_id: pid }) => colaboradoresAcesso?.includes(pid)),
        'perfil_id'
      ),
  ...
  const totalC1 = useMemo(
    () => sumBy(dadosByColaborador?.find(({ item }) => item === colaborador1?.id)?.processos, 'total'),
    [colaborador1?.id, dadosByColaborador]
  );
  const totalC2 = useMemo(
    () => sumBy(dadosByColaborador?.find(({ item }) => item === colaborador2?.id)?.processos, 'total'),
                            ...
                            {colaborador1 && colaborador2 && (
                              <>
                                ...
                                {dadosByAssunto?.map((row) => {
                                  const trabalhadoC1 =
                                    dadosByColaborador
                                      ?.find(({ item }) => item === colaborador1?.id)
                                      ?.processos?.find(({ assunto }) => assunto === row?.item)?.total || 0;
                                  const trabalhadoC2 =
                                    dadosByColaborador
                                      ?.find(({ item }) => item === colaborador2?.id)
                                      ?.processos?.find(({ assunto }) => assunto === row?.item)?.total || 0;
                                  return (
                                    <>
    ...
    <GridItem sm={6}>
      ...
      >
        ...
        <Autocomplete
          ...
          options={colaboradoresList?.filter(({ id }) => id !== colaboradorComp?.id)}
