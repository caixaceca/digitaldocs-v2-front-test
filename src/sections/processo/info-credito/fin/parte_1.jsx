...
export const identificacaoTable = (dados) =>
  gerarTabela({
    ...
    rows: [
      ...
      [1, 1, '2. Identificação do(s) interveniente(s) de crédito (Mutuário(s))', dados?.nome_mutuario ?? 'Mutuário(s)'],
...
export const principaisCaracteristicasTable = (dados) =>
  gerarTabela({
    ...
    rows: [
      [1, 1, '1. Designação comercial do produto', dados?.produto ?? '--'],
      [2, 2, '1.1. Modalidade', '--'],
      [2, 2, '1.2. Finalidade', '--'],
...
export const custosTable = (dados) =>
  gerarTabela({
    ...
    rows: [
      ...
      [2, 2, '1.1. Taxa de juro nominal', dados?.tan ?? '--'],
      ...
      [2, 2, '2.1. TAEG', fPercent(dados?.taeg ?? 0)],
      [2, 2, '2.2. Valor total dos encargos', fCurrency(dados?.valor_total_encargos_iniciais ?? 0)],
